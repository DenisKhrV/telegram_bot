...
public class Scheduler {
    ...
    public void sendModificationTasks() {
        notificationTaskRepository.findAllByExecDate(LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES))
                .forEach(notificationTask -> {
                    SendMessage message = new SendMessage(notificationTask.getChatId(), String.format("Время выполнить задачу: %s", notificationTask.getText()));
